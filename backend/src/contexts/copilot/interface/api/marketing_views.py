...
class SocialAccountsView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        can_edit = access.role(workspace_id=workspace_id, user_id=str(request.user.id)) in ("owner", "admin")
        return Response(
    ...
    def post(self, request: Request) -> Response:
        ...
        workspace_id = self._workspace_id(request)
        access = DjangoWorkspaceAccess()
        if access.role(workspace_id=workspace_id, user_id=str(request.user.id)) not in ("owner", "admin"):
            raise PermissionDeniedError("Apenas dono ou administrador podem conectar contas.")
    ...
    def delete(self, request: Request) -> Response:
        ...
        access = DjangoWorkspaceAccess()
        if access.role(workspace_id=workspace_id, user_id=str(request.user.id)) not in ("owner", "admin"):
            raise PermissionDeniedError("Apenas dono ou administrador podem desconectar contas.")
