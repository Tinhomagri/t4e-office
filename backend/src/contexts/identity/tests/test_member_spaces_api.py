...
def test_members_view_reflete_allowed_spaces(scenario):
    ...
    # owner nunca teve allowed_spaces setado -> null (sem restrição)
    assert by_id[str(scenario["owner"].id)]["allowed_spaces"] is None
...
def test_null_explicito_remove_restricao(scenario):
    ws = scenario["workspace"]
    member = scenario["member"]
    scenario["owner_client"].patch(
        f"/api/auth/workspaces/{ws.id}/members/{member.id}/",
    ...
    assert resp.status_code == 200, resp.data
    assert resp.data["allowed_spaces"] is None
    membership = MembershipModel.objects.get(workspace=ws, user=member)
    assert membership.allowed_spaces is None
